...
class clientAMP(Client):
    ...
    def train(self):
        ...
        for epoch in range(max_local_epochs):
            for x, y in trainloader:
                ...
                gm = torch.cat([p.data.view(-1)
                               for p in self.model.parameters()], dim=0)
                pm = torch.cat([p.data.view(-1)
                               for p in self.client_u.parameters()], dim=0)
    ...
    def set_parameters(self, model, coef_self):
        for new_param, old_param, self_param in zip(model.parameters(), self.client_u.parameters(), self.model.parameters()):
            old_param.data = (new_param.data + coef_self *
                              self_param.data).clone()
